...
namespace Summer.Editor.ResourceEditor
{
    ...
    public sealed class ResourceEditorWindow : EditorWindow
    {
        ...
        private ResourceCollection.Resource m_SelectedResource = null;
        ...
        [MenuItem("Summer/Resource Tools/Resource Editor", false, 42)]
        private static void Open()
        {
            ResourceEditorWindow window = GetWindow<ResourceEditorWindow>("Resource Editor", true);
        ...
        private void OnGUI()
        {
            EditorGUILayout.BeginHorizontal(GUILayout.Width(position.width), GUILayout.Height(position.height));
            {
                ...
                {
                    GUILayout.Space(5f);
                    EditorGUILayout.LabelField(StringUtils.Format("Resource List ({})", m_Controller.ResourceCount.ToString()), EditorStyles.boldLabel);
                ...
                {
                    GUILayout.Space(5f);
                    EditorGUILayout.LabelField(StringUtils.Format("Resource Content ({})", m_CurrentResourceContentCount.ToString()), EditorStyles.boldLabel);
                    EditorGUILayout.BeginHorizontal("box", GUILayout.Height(position.height - 52f));
                    {
                        DrawResourceView();
                    }
        ...
        private void DrawResourcesMenu_Remove()
        {
            ...
            GUILayout.Label(StringUtils.Format("Remove '{}' ?", m_SelectedResource.FullName));
        ...
        private void DrawResourceMenu()
        {
            ...
            {
                if (GUILayout.Button(StringUtils.Format("{} >>", m_SelectedAssetsInSelectedResource.Count.ToString()), GUILayout.Width(80f)))
                {
        ...
        private void DrawSourceAssetsMenu()
        {
            ...
            {
                if (GUILayout.Button(StringUtils.Format("<< {}", selectedSourceAssets.Count.ToString()), GUILayout.Width(80f)))
                {
            ...
            EditorGUI.BeginDisabledGroup(selectedSourceAssets.Count <= 0);
            {
                if (GUILayout.Button(StringUtils.Format("<<< {}", selectedSourceAssets.Count.ToString()), GUILayout.Width(80f)))
                {
                    ...
                    foreach (SourceAsset sourceAsset in selectedSourceAssets)
                    {
                        EditorUtility.DisplayProgressBar("Add Resources", StringUtils.Format("{}/{} processing...", (++index).ToString(), count.ToString()), (float)index / count);
                        ...
                        ResourceCollection.Resource resource = m_Controller.GetResource(name, null);
                        if (resource == null)
                        {
                            continue;
                        }
        ...
        private void ChangeSelectedResource(ResourceCollection.Resource resource)
        {
        ...
        private void AddResource(string name, string variant, bool refresh)
        {
            ...
            if (m_Controller.AddResource(name, variant, null, LoadType.LoadFromFile, false))
            {
                ...
                Debug.Log(StringUtils.Format("Add resource '{}' success.", fullName));
                m_MenuState = MenuState.Normal;
            }
            else
            {
                Debug.LogWarning(StringUtils.Format("Add resource '{}' failure.", fullName));
            }
        }

        private void RenameResource(ResourceCollection.Resource resource, string newName, string newVariant)
        {
            if (resource == null)
            {
            ...
            {
                RefreshResourceTree();
                Debug.Log(StringUtils.Format("Rename resource '{}' to '{}' success.", oldFullName, newFullName));
                m_MenuState = MenuState.Normal;
            }
            else
            {
                Debug.LogWarning(StringUtils.Format("Rename resource '{}' to '{}' failure.", oldFullName, newFullName));
            }
        }
        ...
        private void RemoveResource()
        {
            ...
            if (m_Controller.RemoveResource(m_SelectedResource.Name, m_SelectedResource.Variant))
            {
                ChangeSelectedResource(null);
                RefreshResourceTree();
                Debug.Log(StringUtils.Format("Remove resource '{}' success.", fullName));
            }
            else
            {
                Debug.LogWarning(StringUtils.Format("Remove resource '{}' failure.", fullName));
            }
        }
        ...
        private void SetResourceLoadType(LoadType loadType)
        {
            ...
            {
                Debug.Log(StringUtils.Format("Set resource '{}' load type to '{}' success.", fullName, loadType.ToString()));
            }
            else
            {
                Debug.LogWarning(StringUtils.Format("Set resource '{}' load type to '{}' failure.", fullName, loadType.ToString()));
            }
        }
        ...
        private void SetResourcePacked(bool packed)
        {
            ...
            {
                Debug.Log(StringUtils.Format("{} resource '{}' success.", fullName, packed ? "Pack" : "Unpack"));
            }
            else
            {
                Debug.LogWarning(StringUtils.Format("{} resource '{}' failure.", fullName, packed ? "Pack" : "Unpack"));
            }
        }

        private void AssignAsset(SourceAsset sourceAsset, ResourceCollection.Resource resource)
        {
            if (!m_Controller.AssignAsset(sourceAsset.guid, resource.Name, resource.Variant))
            {
                Debug.LogWarning(StringUtils.Format("Assign asset '{}' to resource '{}' failure.", sourceAsset.name, resource.FullName));
            }
        }
        ...
        private void UnassignAsset(Asset asset)
        {
            ...
            {
                Debug.LogWarning(StringUtils.Format("Unassign asset '{}' from resource '{}' failure.", asset.guid, m_SelectedResource.FullName));
            }
        }
        ...
        private void CleanResource()
        {
            ...
            Debug.Log(StringUtils.Format("Clean complete, {} unknown assets and {} unused resources has been removed.", unknownAssetCount.ToString(), unusedResourceCount.ToString()));
        }

        private void RefreshResourceTree()
        {
            m_ResourceRoot.Clear();
            ResourceCollection.Resource[] resources = m_Controller.GetResources();
            foreach (ResourceCollection.Resource resource in resources)
            {
                ...
                string fullName = resource.Variant != null ? StringUtils.Format("{}.{}", splitedPath[splitedPath.Length - 1], resource.Variant) : splitedPath[splitedPath.Length - 1];
