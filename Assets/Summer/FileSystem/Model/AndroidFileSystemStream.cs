...
namespace Summer.FileSystem.Model
{
    ...
    public sealed class AndroidFileSystemStream : FileSystemStream
    {
        ...
        public AndroidFileSystemStream(string fullPath, FileSystemAccess access, bool createNew)
        {
            ...
            if (access != FileSystemAccess.Read)
            {
                throw new GameFrameworkException(
                    StringUtils.Format("'{}' is not supported in AndroidFileSystemStream.", access.ToString()));
            }
            ...
            {
                throw new GameFrameworkException(
                    StringUtils.Format("Open file '{}' from Android asset manager failure.", fullPath));
            }
        ...
        private int InternalRead(int length, out byte[] result)
        {
            ...
            var sbytes = AndroidJNI.FromSByteArray(resultPtr);
