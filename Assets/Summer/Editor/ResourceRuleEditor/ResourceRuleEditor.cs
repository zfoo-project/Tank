...
using Summer.Editor.ResourceRuleEditor;
using Summer.Resource.Model.Constant;
using UnityEditor;
using UnityEditorInternal;
using UnityEngine;
using GFResource = Summer.Editor.ResourceCollection.Resource;
...
namespace StarForce.Editor.ResourceTools
{
    ...
    {
        private readonly string m_ConfigurationPath = "Assets/ResourceRuleEditor.asset";
        ...
        [MenuItem("Summer/Resource Tools/Resource Rule Editor", false, 43)]
        static void Open()
        {
                    ...
                    switch (resourceRule.filterType)
                    {
                        case ResourceFilterType.Root:
                        {
                            if (string.IsNullOrEmpty(resourceRule.name))
                            {
                                ...
                                ApplyResourceFilter(ref signedAssetBundleList, resourceRule, PathUtils.GetRegularPath(relativeDirectoryName));
                            }
                            else
                            {
                            ...
                            for (int i = 0; i < patterns.Length; i++)
                            {
                                ...
                                foreach (FileInfo file in assetFiles)
                                {
                                    ...
                                    string relativeAssetNameWithoutExtension =
                                        PathUtils.GetRegularPath(relativeAssetName.Substring(0, relativeAssetName.IndexOf('.')));
                        ...
                        case ResourceFilterType.ChildrenFoldersOnly:
                        {
                            ...
                            foreach (DirectoryInfo directory in assetDirectories)
                            {
                                ...
                                ApplyResourceFilter(ref signedAssetBundleList, resourceRule,
                                    PathUtils.GetRegularPath(relativeDirectoryName), string.Empty, directory.FullName);
                            }
                        }
                            break;
