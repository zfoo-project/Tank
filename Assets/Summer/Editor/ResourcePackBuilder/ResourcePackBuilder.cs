...
namespace Summer.Editor.ResourcePackBuilder
{
    ...
    /// 资源包生成器。
    /// </summary>
    public sealed class ResourcePackBuilder : EditorWindow
    {
        ...
        [MenuItem("Summer/Resource Tools/Resource Pack Builder", false, 44)]
        ...
        private string GetVersionNameForDisplay(string versionName)
        {
            ...
            return StringUtils.Format("{} ({})", text, splitedVersionNames[splitedVersionNames.Length - 1]);
        }

        private void OnBuildResourcePacksStarted(int count)
        {
            Debug.Log(StringUtils.Format("Build resource packs started, '{}' items to be built.", count.ToString()));
            EditorUtility.DisplayProgressBar("Build Resource Packs", StringUtils.Format("Build resource packs, {} items to be built.", count.ToString()), 0f);
        }

        private void OnBuildResourcePacksCompleted(int successCount, int count)
        {
            int failureCount = count - successCount;
            string str = StringUtils.Format("Build resource packs completed, '{}' items, '{}' success, '{}' failure.", count.ToString(), successCount.ToString(), failureCount.ToString());
            if (failureCount > 0)
            {
        ...
        {
            Debug.Log(StringUtils.Format("Build resource packs success, source version '{}', target version '{}'.", GetVersionNameForDisplay(sourceVersion), GetVersionNameForDisplay(targetVersion)));
            EditorUtility.DisplayProgressBar("Build Resource Packs", StringUtils.Format("Build resource packs, {}/{} completed.", (index + 1).ToString(), count.ToString()), (float)index / count);
        }

        private void OnBuildResourcePackFailure(int index, int count, string sourceVersion, string targetVersion)
        {
            Debug.LogWarning(StringUtils.Format("Build resource packs failure, source version '{}', target version '{}'.", GetVersionNameForDisplay(sourceVersion), GetVersionNameForDisplay(targetVersion)));
            EditorUtility.DisplayProgressBar("Build Resource Packs", StringUtils.Format("Build resource packs, {}/{} completed.", (index + 1).ToString(), count.ToString()), (float)index / count);
        }
    }
}
